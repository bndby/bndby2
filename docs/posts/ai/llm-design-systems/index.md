...
LLM отклоняются от системы, придумывают токены и начинают каждый сеанс с чистого листа. Вот как передать дизайн-систему ИИ-агентам, чтобы они перестали гадать.

<!-- more -->

## Кратко

Ваша дизайн-система уже существует в коде: библиотеки компонентов, файлы токенов и переменные Figma. Проблема в том, что при вайб-кодинге LLM не получают к ней надёжного доступа. Они придумывают имена токенов, внутри одного сеанса используют разные значения для похожих элементов, теряют контекст между сеансами и не замечают несовместимые изменения в вышестоящей библиотеке.

Описанный здесь метод превращает дизайн-систему в формат, который LLM могут использовать последовательно: структурированные файлы спецификаций, ограниченный набор токенов и автоматический аудит, выявляющий каждое отклонение.
...
1.  **Они придумывают значения.** LLM не ищут токены вашей дизайн-системы. Они генерируют правдоподобные значения. Если система использует `--space-200` для `8px`, LLM может написать `padding: 12px`, потому что `12px` — разумное число. Это не обязательно неправильно. Просто это не ваше значение.
...
3.  **Они не могут прочитать замысел дизайна из исходного кода.** Если показать LLM библиотеку компонентов вроде [Atlaskit](https://atlassian.design/), она увидит API: импортировать `Button`, передать `appearance="primary"` — готово. Но из исходного кода она не извлечёт:
    ...
    - как составлять их в макеты по вашим соглашениям.
...
Вместо того чтобы решать, «каким синим должна быть эта ссылка», LLM читает файл спецификации и находит `var(--color-link)`. Вместо придумывания отступа она читает справочник токенов и находит `var(--space-200)`. Дизайнерское решение уже принято человеком — LLM лишь находит его.
...
2.  **Ограниченный слой токенов, из которого LLM выбирает значения.** Решает проблему придуманных значений. Вместо `padding: 16px`, разбросанного по 30 файлам, вы создаёте `var(--space-200)` и используете его везде. LLM выбирает из ограниченного набора именованных переменных, а не изобретает правдоподобные значения.
...
Спецификации превращают неявные дизайнерские решения в явные инструкции. Слой токенов заменяет выбор значения выбором имени. Аудит превращает согласованность из вопроса вкуса в проверяемое ограничение. Вместе они делают дизайн-систему воспроизводимой: одинаковой внутри сеанса, между сеансами и между разными разработчиками.
...
Проверь результат, подстрой значения токенов под свой вкус и создай коммит. В итоге вы получите:
...
!!!tip "Обратите внимание"
    ...
    Смотрите на файлы с наибольшим количеством жёстко заданных значений. Именно там больше всего догадок.
...
Три слоя адресации выглядят так:

| Слой | Пример | Назначение |
| --- | --- | --- |
| Вышестоящая дизайн-система | `--ds-text: #292a2e;` | Источник истины, который может обновиться |
| Проектный псевдоним | `--color-text: var(--ds-text, #292a2e);` | Единая точка переключения тем и версий |
| Компонент | `color: var(--color-text);` | Потребление токена без привязки к реализации |
...
Каждый файл следует единому шаблону из восьми разделов: метаданные, обзор, анатомия, токены, props/API, состояния, примеры кода и перекрёстные ссылки.
...
**Файл инструкций проекта** требует обращаться к спецификациям перед любой работой над UI. Он читается в начале каждого сеанса с ИИ.

**Скрипт аудита токенов** сканирует CSS-файлы, находит жёстко заданные значения и предлагает правильный токен. При ошибках он возвращает код выхода 1, поэтому его можно добавить в CI.
...
**Чек-лист проверки дизайна** охватывает жёстко заданные значения, полноту состояний, анатомию компонентов, согласованность отступов, типографику, анимации, доступность и документирование отклонений.

**Шаг 5: Заменить каждое жёстко заданное значение токеном**

Промпт проходит по каждому CSS-файлу и заменяет жёстко заданные значения созданными токенами. Теперь каждое `color:`, `padding:`, `border-radius:` и `font-size:` проходит через `var(--token)`.
...
Подход не заменяет дизайнера. Если в дизайн-системе изначально нет чётких правил, LLM унаследует эту неопределённость в виде токенов и спецификаций. Если команда не запускает аудит в CI, отклонения будут проскальзывать. Если спецификации не обновляют при изменении компонентов, LLM будет читать устаревшие инструкции. Метод усиливает существующую дисциплину, а не создаёт её с нуля.
...
Прототипы, созданные вайб-кодингом, разваливаются после нескольких сеансов, потому что LLM незаметно накапливают ошибки. Каждый сеанс добавляет новые придуманные значения, новые несогласованности и новые отклонения от исходной дизайн-системы. К десятому сеансу прототип выглядит как три разных продукта.

Понятная дизайн-система ограничивает LLM в каждой точке, где иначе пришлось бы гадать. Спецификации дают ей память между сеансами. Слой токенов предоставляет ограниченный набор значений вместо возможности придумывать новые. Аудит выявляет всё, что проскользнуло, а обнаружение отклонений поддерживает актуальность относительно вышестоящей системы.
...
Числа менее важны, чем изменения в рабочем процессе. Мы перестали проверять визуальную согласованность результата LLM, потому что ограничения делают это за нас. LLM читает спецификацию, использует токен, а аудит выявляет всё пропущенное. Человеческий вкус задаётся один раз. После этого LLM механически ему следует.

### Что изменилось в практике

- Ревью UI теперь начинаются с прогона аудита: если он показывает ноль, обсуждение смещается с «правильного ли здесь оттенок» к логике компонента.
- Новые компоненты создаются по шаблону спецификации, а не по памяти последнего сеанса.
- Обновления вышестоящей библиотеки больше не приводят к тихим визуальным регрессиям: процедура синхронизации указывает, какие спецификации проверить.
...
Файлы спецификаций находятся в репозитории рядом с кодом, который они описывают, а не в wiki или комментариях Figma. Когда компонент меняется в PR, файл спецификации попадает в тот же diff. Процедура обнаружения отклонений отмечает обновления вышестоящей дизайн-системы, которые затрагивают ваши спецификации.
...
Дизайн-система, понятная LLM, — это не отдельный артефакт, а три связанных механизма: спецификации дают память, токены ограничивают выбор, аудит ловит отклонения. Вместе они превращают вайб-кодинг из последовательности догадок в воспроизводимый процесс. Человек принимает дизайнерские решения один раз; LLM применяет их каждый раз.
