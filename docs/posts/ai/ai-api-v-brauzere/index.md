...
async function translateForSupport(inputText, targetLanguage = "ru") {
  ...
  if (detectorAvailability === "unavailable" || translatorAvailability === "unavailable") {
    return serverTranslate(inputText, targetLanguage);
  }
...
async function summarizeArticle(articleEl) {
  ...
  if (availability === "unavailable") return serverSummarize(articleEl.innerText);

  if (!navigator.userActivation.isActive) {
    throw new Error("Клик по кнопке 'Сжать текст' должен быть user-initiated");
  }
...
async function classifyFeedback(feedbackText) {
  ...
  if (!navigator.userActivation.isActive) throw new Error("User activation required");
