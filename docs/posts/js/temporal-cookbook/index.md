...
(Чтобы вернуть объект `Temporal` из элемента управления «Календарь», см. пример [«Сколько дней до будущей даты»](#сколько-дней-до-будущей-даты).)
...
См. также раздел [«Отодвинуть дату запуска»](#отодвинуть-дату-запуска), чтобы узнать более простой способ безусловного округления до _следующего_ начала месяца.
